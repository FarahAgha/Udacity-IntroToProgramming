...
def blank_word(blank_word, list_answer):
    num = blank_number(blank_word)
    assert num >0 and num <= len(list_answer),"invalid number in blank space, Check words in the list"
    return list_answer[num-1][1]

def replace_blank(list_quiz,list_answer):
    ...
    for word in quiz_string:
        ...
        if word[0] =='_':
            if word[-1] == '_':
                word = blank_word(word, list_answer)
...
def display_incorrect(list_ch, list_user):
    list_length =  len(list_user)
    count =0
    while count < list_length:
        # Done TO DO ignore Case for user inputs
        if list_user[count][1].upper() != list_ch[1][count].upper():
            print "Incorrect Value ", list_user[count][1], " at location ",list_user[count][0]+1
        count = count + 1
...
def launch_game(list_ch):
    ...
    list_user = user_list_answer(list_ch[rand_int])
    # done TO DO display incorrect answeres if any/ congratulation message
    display_incorrect(list_ch[rand_int], list_user)
    replace_blank(list_ch[rand_int],list_user)
